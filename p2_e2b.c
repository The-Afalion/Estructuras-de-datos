...
/**
 * @brief Runs exercise 2B.
 *
 * The function reads two playlists ordered by artist, loads both playlists
 * into two stacks, merges them using the artist comparator, and prints the
 * result.
 *
...
int main(int argc, char **argv) {
    ...
    if (argc < MIN_ARGC) {
        fprintf(stderr, "Usage: %s <playlistA.txt> <playlistB.txt>\n", argv[0]);
        return ERROR_MAIN;
    }
    ...
    sout = stack_init();
    if (!sout || mergeStacks(s1, s2, sout, music_artist_cmp) == ERROR) {
